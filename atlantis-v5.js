...
function initGalleryParallax() {
  const items = document.querySelectorAll('.at-gallery-item');
  if (!items.length) return;

  items.forEach((item, index) => {
    // Las imágenes pares bajan ligeramente, las impares suben ligeramente durante el scroll
...
function initAudio() {
  ...
  // Arrancar ambient audio en la primera interacción para cumplir política de navegadores
  const startAudioInteraction = () => {
    if(ambient.paused) {
      ambient.volume = 0.25;
      ambient.play().catch(() => {});
    }
    window.removeEventListener('scroll', startAudioInteraction, { passive: true });
    window.removeEventListener('touchstart', startAudioInteraction, { passive: true });
    window.removeEventListener('click', startAudioInteraction, { passive: true });
  };

  window.addEventListener('scroll', startAudioInteraction, { passive: true });
  window.addEventListener('touchstart', startAudioInteraction, { passive: true });
  window.addEventListener('click', startAudioInteraction, { passive: true });
