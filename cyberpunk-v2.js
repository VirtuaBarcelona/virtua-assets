function initCyberpunk() {
  ...
  // 9. STICKY BUTTON LOGIC (STANDARD MÓVIL)
  ...
  if (stickyBtn && heroSection && ctaSection) {
    ...
    // Ocultar botón flotante cuando el hero es visible
    stickyObserver.observe(heroSection);
    
    const possibleFooters = document.querySelectorAll('footer, .elementor-location-footer, .site-footer, .site-below-footer-wrap, .cp-cta-section');
    if (possibleFooters.length > 0) {
      ...
      possibleFooters.forEach(f => footerObserver.observe(f));
    }
  }

  // 10. GSAP MOBILE REFRESH
  let resizeTimer;
  ...
  // 11. MOBILE GALLERY OBSERVER
  if (window.innerWidth <= 768) {
    const galleryItemsMobile = document.querySelectorAll('.cp-gallery-item');
    if (galleryItemsMobile.length > 0) {
      const galleryObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('cp-active');
          } else {
            entry.target.classList.remove('cp-active');
          }
        });
      }, {
