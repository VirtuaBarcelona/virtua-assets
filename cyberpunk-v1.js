function initCyberpunk() {
  ...
  // 6. SYSTEM OVERLOAD SCROLL GLITCH
  let lastGlitchTime = 0;
  window.addEventListener('scroll', () => {
    const now = Date.now();
    // Random chance to glitch when scrolling, but not more than once every 3 seconds
    if (Math.random() < 0.02 && now - lastGlitchTime > 3000) {
      lastGlitchTime = now;
