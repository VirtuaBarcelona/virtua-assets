...
(function() {
  ...
  if (window.innerWidth <= 768) {
    const galleryItems = document.querySelectorAll('.xmas-gallery-item');
    if (galleryItems.length > 0) {
      const galleryObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('xmas-active');
          } else {
            entry.target.classList.remove('xmas-active');
          }
        });
      }, {
        root: null,
