// ======================================================================
// SURVIVAL — LÓGICA GSAP & INTERACTIVIDAD v7.0 (PERFECTION ITERATION)
// ======================================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    gsap.set("#srv-heli-light", { opacity: 0 });
    ...
    tl.to("#srv-fg-left", { 
        xPercent: -120, rotation: -10, scale: 1.2, duration: 3, ease: "power2.inOut",
        onStart: () => { if (!playedLeaves) { audioLeaves.play(); playedLeaves = true; } }
    }, 0);
    tl.to("#srv-fg-right", { xPercent: 120, rotation: 10, scale: 1.2, duration: 3, ease: "power2.inOut" }, 0);
    ...
    // Typewriter effect after it scales in
    tl.fromTo("#srv-pre-title", 
        { text: "" }, 
        { text: "Vuestro avión se estrelló.<br>¿Estáis listos para la aventura?", duration: 2, ease: "none" }, 
        2.5
    );

    tl.to({}, { duration: 1.5 }); // Pausa
    tl.to("#srv-pre-title", { opacity: 0, scale: 0.9, duration: 1.5 });
    tl.to("#srv-title", { opacity: 1, scale: 1, duration: 2, ease: "power1.out" }, "+=0.2");
    ...
    const yPos = [-80, -120, 40, 80, -20, 150];
    ...
    tl.to("#srv-friction-text", { opacity: 1, duration: 1 }, "-=1");
    ...
    document.addEventListener('touchstart', (e) => {
        if (!e.target.closest('.srv-polaroid')) {
            pols.forEach(p => gsap.to(p, { scale: 0.75, zIndex: 25, duration: 0.3 }));
        }
    }, { passive: true });
    ...
    function triggerFinalCinematic() {
        ...
        finalTl.to("#srv-heli-light", { opacity: 1, duration: 1 }, "-=4");
