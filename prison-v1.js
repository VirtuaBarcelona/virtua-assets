...
function initLenis() {
    ...
    // Audio unlock — probe on every interaction, retries if browser rejects (e.g. wheel without click)
...
function initIntroScreen() {
    ...
    // Generar rejas (14 verticales + 2 horizontales)
    const barsWrap = intro.querySelector('.intro-bars-wrap');
    if (barsWrap) {
        for (let i = 0; i < 14; i++) {
    ...
    // Bloquear scroll hasta que el usuario entre
    ...
    function enter() {
        ...
        // Audio — contexto de click real garantizado, sin probe necesario
        if (!_audioInited) {
            _audioInited       = true;
            state.audioEnabled = true;
            _UNLOCK_EVENTS.forEach(ev => window.removeEventListener(ev, _tryInitAudio));
        ...
        // Animación: reja subiendo (gate opening)
        gsap.to(intro, {
            y: '-100%',
            duration: 0.85,
            ease: 'power2.inOut',
