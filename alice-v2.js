    ...
    try {
        ...
        gsap.registerPlugin(ScrollTrigger);
                ...
                crystallizeTargets.forEach(el => {
                    ...
                    gsap.to(el, { scrollTrigger: { trigger: el, start: "top 92%", toggleActions: "play none none none", invalidateOnRefresh: true }, opacity: 1, letterSpacing: finalSpacing, scale: 1, y: 0, duration: 1.5, ease: "expo.out" });
                });
                ...
                bottles.forEach((b, i) => {
                    gsap.from(b, { scrollTrigger: { trigger: '#alice-sec-data', start: "top 70%", toggleActions: "play none none reverse", invalidateOnRefresh: true }, height: 5, opacity: 0, duration: 0.6, delay: i * 0.08, ease: "back.out(2)" });
                });
                ...
                document.querySelectorAll('.alice-page-turn').forEach(pt => {
                    gsap.from(pt, { scrollTrigger: { trigger: pt, start: "top 85%", toggleActions: "play none none none", invalidateOnRefresh: true }, scaleX: 0, opacity: 0, duration: 1.2, ease: "power2.inOut" });
                });

                document.querySelectorAll('.alice-chapter').forEach((ch, i) => {
                    const body = ch.querySelector('.alice-chapter-body'); if (!body) return;
                    const isEven = i % 2 === 1;
                    gsap.from(body, { scrollTrigger: { trigger: ch, start: "top 80%", toggleActions: "play none none none", invalidateOnRefresh: true }, rotateY: isEven ? -8 : 8, opacity: 0.3, x: isEven ? 60 : -60, duration: 1.4, ease: "power3.out" });
                });
                ...
                // Force refresh on load to prevent miscalculations
                window.addEventListener('load', () => {
                    setTimeout(() => ScrollTrigger.refresh(true), 500);
                });

    } catch(err) {
        const t = document.querySelector('.alice-intro-text');
        if(t) { t.innerText = 'JS ERROR: ' + err.message + ' | Line: ' + err.stack; t.style.color = 'red'; t.style.opacity = 1; }
        console.error(err);
    }
});
