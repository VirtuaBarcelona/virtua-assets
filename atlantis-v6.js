...
function fireSonarPing(x = '50%', y = '50%') {
  ...
  if (sonarAudio) {
    sonarAudio.volume = window._atSonarVol ?? 0.40;
...
function triggerSectionReveal(sectionId) {
  ...
  gsap.fromTo(section.querySelectorAll('.at-kpi, .at-kpi-value-row, .at-glass-panel, .at-gallery-item, .at-section-title'), {
    opacity: 0, y: 30
  }, {
    opacity: 1, y: 0,
    stagger: 0.15,
...
function initDepthScroll() {
  ...
  const rootEl = document.getElementById('at-root');
  const isEn = document.documentElement.lang.startsWith('en') || (rootEl && rootEl.classList.contains('translate-en'));

  const DEPTH_STAGES = isEn ? [
    { depth: 0,    stage: 0, status: 'SURFACE',                     section: null },
    { depth: 200,  stage: 1, status: 'TWILIGHT ZONE',               section: null },
    { depth: 700,  stage: 2, status: 'MIDNIGHT ZONE · INTEL ACTIVE',section: 'at-briefing' },
    { depth: 1400, stage: 3, status: 'ABYSSAL ZONE · ARCHIVES',     section: 'at-gallery-section' },
    { depth: 2100, stage: 4, status: 'BOTTOM REACHED',              section: 'at-cta-section' },
    { depth: 2450, stage: 4, status: 'STRUCTURE CONFIRMED',         section: null },
  ] : [
...
function initGalleryParallax() {
  ...
  if (isTouch) {
    // Móvil: cada ítem se revela individualmente al pasar por el centro
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(e => e.target.classList.toggle('at-active', e.isIntersecting));
    }, { rootMargin: '-25% 0px -25% 0px' });
    document.querySelectorAll('.at-gallery-item').forEach(el => observer.observe(el));
    return;
  }

  // Desktop: parallax stagger — ítems pares bajan, impares suben, pero también tienen hover
  // Sin pin (4 ítems no justifican el scroll stop)
  document.querySelectorAll('.at-gallery-item').forEach((item, index) => {
    const yDir = (index % 2 === 0) ? 40 : -40;
    gsap.fromTo(item, { y: yDir }, {
      y: -yDir,
      ease: 'none',
      scrollTrigger: {
        trigger: '#at-gallery-section',
        start: 'top bottom',
        end: 'bottom top',
        scrub: true
      }
...
function initWebGL(activeVideo) {
  ...
  let mouseTurbulence = 0;
  window.addEventListener('mousemove', (e) => {
    const movement = Math.sqrt(e.movementX ** 2 + e.movementY ** 2);
    mouseTurbulence += movement * 0.01;
    if (mouseTurbulence > 1.5) mouseTurbulence = 1.5;
  });
  ...
  function draw(time) {
    try {
      ...
      // Decay mouse turbulence
      mouseTurbulence *= 0.92;
      let totalTurbulence = mouseTurbulence;
      if (turbLoc && turbLoc._scrollTurb) {
        totalTurbulence += turbLoc._scrollTurb;
      }
      
      gl.uniform1f(turbLoc, Math.min(totalTurbulence, 1.0));
      
      gl.drawArrays(gl.TRIANGLES, 0, 6);
    } catch (e) {
      console.warn('WebGL render error (posible CORS):', e);
      // Fallback: make video visible if WebGL crashes
...
function initAudio() {
  ...
  // Volúmenes diferenciados por dispositivo
  const isMobile  = window.matchMedia('(pointer: coarse)').matches;
  const BASE_VOL  = isMobile ? 0.08 : 0.22;
  const MAX_VOL   = isMobile ? 0.22 : 0.55;
  const SONAR_VOL = isMobile ? 0.12 : 0.40;

  // Exponer SONAR_VOL a fireSonarPing (que vive en el scope global del fichero)
  window._atSonarVol = SONAR_VOL;

  // Arrancar ambient en la primera interacción del usuario
  const startAudio = () => {
    if (ambient.paused) {
      ambient.volume = BASE_VOL;
      ambient.play().catch(() => {});
    }
    window.removeEventListener('scroll',     startAudio, { passive: true });
    window.removeEventListener('touchstart', startAudio, { passive: true });
    window.removeEventListener('click',      startAudio, { passive: true });
  };
  ...
  // Volumen del ambient aumenta con la profundidad (fade in progresivo)
  ScrollTrigger.create({
    trigger: '#at-content',
    start: 'top top',
    end: 'bottom bottom',
    ...
    onUpdate(self) {
      if (!ambient.paused) {
        ambient.volume = BASE_VOL + self.progress * (MAX_VOL - BASE_VOL);
      }
    }
  });
}
...
function initLenis() {
  ...
  lenis.on('scroll', ({ velocity }) => {
    ...
    if (gl && turbLoc) {
      turbLoc._scrollTurb = turbulence;
    }
  });
