...
(function() {
  ...
  function initAnimations() {
    ...
    // -- Mobile Gallery Active State (Vertical scroll center detection) --
    if (window.innerWidth <= 768) {
      const galleryItems = document.querySelectorAll('.ch-gallery-item');
      if (galleryItems.length > 0) {
        const galleryObserver = new IntersectionObserver((entries) => {
          entries.forEach(entry => {
            if (entry.isIntersecting) {
              entry.target.classList.add('ch-active');
            } else {
              entry.target.classList.remove('ch-active');
            }
          });
        }, {
          root: null,
